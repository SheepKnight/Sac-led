...
uint16_t * rightBuffer;
uint16_t * leftBuffer;
char * trackInfo[4] = { NULL };
void(*displayInfos)(char * track, char * artist, char * disc, char * style);

void setupBuf(uint16_t * rBuffer, uint16_t * lBuffer, void(*newDisplayInfos)(char * track, char * artist, char * disc, char * style)) {
	rightBuffer = rBuffer;
	leftBuffer = lBuffer;
	displayInfos = newDisplayInfos;
}
...
void bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
	...
	if(len > 4){
		*rightBuffer = ((uint16_t)data[1] << 8) | data[0];
		*leftBuffer = ((uint16_t)data[3] << 8) | data[2];
	}
	if (++s_pkt_cnt % 100 == 0) {
        ESP_LOGI(BT_AV_TAG, "Audio packet count %u, on core %u", s_pkt_cnt, xPortGetCoreID());
...
static void bt_av_hdl_avrc_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
		...
		int size = 1+strlen((char*)rc->meta_rsp.attr_text);
		ESP_LOGI(BT_AV_TAG, "%u", size);
		switch(rc->meta_rsp.attr_id){
		
			case 1: {
				trackInfo[0]=malloc(size * sizeof(char));
				memcpy(trackInfo[0], rc->meta_rsp.attr_text, size);
				break;
			}
			case 2: {
				trackInfo[1] = malloc(size * sizeof(char));
				memcpy(trackInfo[1], rc->meta_rsp.attr_text, size);
				break;
			}
			case 4: {
				trackInfo[2] = malloc(size * sizeof(char));
				memcpy(trackInfo[2], rc->meta_rsp.attr_text, size);
				break;
			}
			case 32: {
				trackInfo[3] = malloc(size * sizeof(char));
				memcpy(trackInfo[3], rc->meta_rsp.attr_text, size);
				
				displayInfos(trackInfo[0], trackInfo[1], trackInfo[2], trackInfo[3]);
				break;
			}
